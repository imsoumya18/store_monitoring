...
def get_status_timerange(menu_hours: Dict[str, List],
    ...
    def check_time_in_range(time, weekday):
        ...
        for i in range(0, len(menu_hours[weekday])):
            if str(time) >= menu_hours[weekday][i][0] and str(time) <= menu_hours[weekday][i][1]:
                return False
        return True
    ...
    sum_active_hours, sum_inactive_hours = sub_status_timerange['active_hours'].sum(), sub_status_timerange[
        'inactive_hours'].sum()
